...
toggle_status() {
    if tailscale_status; then
        tailscale down
        notify-send "Tailscale" "VPN disabled"
    else
        tailscale up
        notify-send "Tailscale" "VPN enabled"
    fi
    sleep 5
}
...
case $1 in
    --status)
        if tailscale_status; then
            ...
            echo "{\"text\":\"vpn: ${exitnode:-none}\",\"class\":\"connected\",\"alt\":\"connected\", \"tooltip\": \"${peers}\"}"
        else
            echo "{\"text\":\"vpn: <span foreground=\\\"#666666\\\">off</span>\",\"class\":\"stopped\",\"alt\":\"stopped\", \"tooltip\": \"The VPN is not active.\"}"
        fi
    ;;
    --toggle)
        toggle_status
